...
class ActuatorMode(Enum):
    ...
    ONE_THRUSTER_RW = "one_thruster_rw"  # RW + forward/brake thrusters
...
class ActuatorConfig:
    ...
    mode: ActuatorMode = ActuatorMode.ONE_THRUSTER_RW
    ...
    def __post_init__(self):
        ...
        # Ensure mode is ActuatorMode enum
        if isinstance(self.mode, str):
            self.mode = ActuatorMode(self.mode)

        if self.mode in [ActuatorMode.REACTION_WHEELS, ActuatorMode.ONE_THRUSTER_RW]:
            ...
            if self.thrusters is None:
                if self.mode == ActuatorMode.ONE_THRUSTER_RW:
                    # Forward + brake thrusters along the X axis
                    # Rear face is at x=-0.145 (push +X), front at x=+0.145 (push -X)
                    self.thrusters = ThrusterSetConfig(
                        positions={"main": (-0.145, 0.0, 0.0), "brake": (0.145, 0.0, 0.0)},
                        directions={"main": (1.0, 0.0, 0.0), "brake": (-1.0, 0.0, 0.0)},
                    )
                else:
                    self.thrusters = ThrusterSetConfig()
    ...
    def control_dimension(self) -> int:
        ...
        elif self.mode == ActuatorMode.ONE_THRUSTER_RW:
            return 5  # 3 RW torques + 2 thruster forces
        else:
    ...
    def model_path(self) -> str:
        ...
        if self.mode == ActuatorMode.REACTION_WHEELS:
            ...
            return "models/satellite_1thruster_rw.xml"
        else:
    ...
    def get_control_labels(self) -> list:
        ...
        if self.mode == ActuatorMode.REACTION_WHEELS:
            ...
            # 3 RW torques + 2 thrusters (main + brake)
            return ["τ_rw_x", "τ_rw_y", "τ_rw_z", "F_main", "F_brake"]
        else:
...
def create_actuator_config(
    mode: Literal["reaction_wheels", "legacy_thrusters", "one_thruster_rw"] = "one_thruster_rw",
) -> ActuatorConfig:
    """
...
DEFAULT_ACTUATOR_MODE: ActuatorMode = ActuatorMode.ONE_THRUSTER_RW
