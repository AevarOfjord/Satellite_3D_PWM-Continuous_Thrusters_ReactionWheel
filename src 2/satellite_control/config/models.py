...
class SatellitePhysicalParams(BaseModel):
    ...
    thruster_positions: Dict[int, Tuple[float, float, float]] = Field(
        ...,
        description="Map of thruster ID (1..N) to (x, y, z) position in meters",
    )
    thruster_directions: Dict[int, Tuple[float, float, float]] = Field(
        ...,
        description="Map of thruster ID (1..N) to (dx, dy, dz) unit direction vector",
    )
    thruster_forces: Dict[int, float] = Field(
        ...,
        description="Map of thruster ID (1..N) to max force in Newtons",
    )
    ...
    def validate_thruster_positions(
        ...
        if ids != list(range(1, len(ids) + 1)):
            raise ValueError(f"Thruster IDs must be contiguous starting at 1, got {ids}")
    ...
    def validate_thruster_forces(cls, v: Dict[int, float]) -> Dict[int, float]:
        ...
        for tid, force in v.items():
            if tid < 1:
                raise ValueError(f"Thruster ID must be >= 1, got {tid}")
            if force <= 0:
    ...
    def validate_thruster_id_consistency(self) -> "SatellitePhysicalParams":
        """Ensure thruster ID sets match across positions, directions, and forces."""
        pos_ids = set(self.thruster_positions.keys())
        dir_ids = set(self.thruster_directions.keys())
        force_ids = set(self.thruster_forces.keys())
        if pos_ids != dir_ids or pos_ids != force_ids:
            raise ValueError(
                "Thruster IDs must match across positions, directions, and forces "
...
class SimulationParams(BaseModel):
    ...
    headless: bool = Field(
        True,  # Default to headless for performance
...
from .actuator_config import ActuatorConfig, create_actuator_config
