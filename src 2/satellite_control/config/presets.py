...
class ConfigPreset:
    ...
    STABLE = "stable"
    PRECISION = "precision"
    ONE_THRUSTER = "one_thruster"

    @classmethod
    def all(cls) -> list[str]:
        """Get all available preset names."""
        return [cls.FAST, cls.BALANCED, cls.STABLE, cls.PRECISION, cls.ONE_THRUSTER]
...
def load_preset(preset_name: str) -> Dict[str, Any]:
    ...
    elif preset_name == ConfigPreset.ONE_THRUSTER:
        # Use stable MPC params as base for single thruster
        mpc = _create_stable_mpc()
        # Reduce angle weight significantly to allow "turn-burn-turn" maneuvers
        # The satellite must rotate to aim the thruster, so strict angle holding
        # prevents translation.
        mpc.q_angle = 10.0  # Reduced from 1000.0
        # Increase position weight to prioritize reaching the target
        mpc.q_position = 2000.0
        actuator_mode = "one_thruster_rw"
    else:
        available = ", ".join(ConfigPreset.all())
        raise ValueError(f"Invalid preset name '{preset_name}'. " f"Available presets: {available}")
    ...
    if actuator_mode:
        overrides["actuator"] = {
            "mode": actuator_mode,
            "thrusters": None,  # Force re-initialization in __post_init__
        }
...
def get_preset_description(preset_name: str) -> str:
    ...
    descriptions = {
        ...
        ConfigPreset.ONE_THRUSTER: (
            "One Thruster preset: Activates 'Point-and-Shoot' mode with 1 Thruster + 3 Reaction Wheels. "
            "Uses stable MPC parameters."
        ),
    }
