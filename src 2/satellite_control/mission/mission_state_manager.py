...
from src.satellite_control.utils.orientation_utils import (
    closest_symmetric_quat,
    euler_xyz_to_quat_wxyz,
    quat_angle_error,
    quat_angle_error_symmetric,
)
...
class MissionStateManager:
    ...
    def __init__(
        ...
        if mission_state is None:
            raise ValueError("mission_state is required (V4.0.0: no SatelliteConfig fallback)")
        if app_config is None:
            raise ValueError("app_config is required (V4.0.0: no SatelliteConfig fallback)")
        ...
        self.use_symmetric_angle_error = (
            self.app_config.actuator.mode
            in [ActuatorMode.REACTION_WHEELS, ActuatorMode.ONE_THRUSTER_RW]
            if self.app_config and self.app_config.actuator
            else False
        )
    ...
    def _quat_angle_error(self, q_target: np.ndarray, q_current: np.ndarray) -> float:
        """Quaternion angle error with optional symmetry handling."""
        if self.use_symmetric_angle_error:
            return quat_angle_error_symmetric(q_target, q_current)
        return quat_angle_error(q_target, q_current)

    def _apply_symmetry_to_target_state(
        self,
        target_state: Optional[np.ndarray],
        current_quat: np.ndarray,
    ) -> Optional[np.ndarray]:
        """Snap target quaternion to the closest symmetric equivalent."""
        if target_state is None:
            return None
        if self.use_symmetric_angle_error:
            target_state = target_state.copy()
            target_state[3:7] = closest_symmetric_quat(target_state[3:7], current_quat)
        return target_state
    ...
    def get_trajectory(
        ...
        # --- GUIDANCE OVERRIDE FOR ONE_THRUSTER MODE ---
        from src.satellite_control.config.actuator_config import ActuatorMode

        if (
            hasattr(self.app_config, "actuator")
            and self.app_config.actuator.mode == ActuatorMode.ONE_THRUSTER_RW
        ):
            # In One Thruster mode, we MUST align the body +X axis with the target vector
            # otherwise the Linear MPC sees no thrust authority in the required direction.

            # Use the final trajectory point as the goal
            goal_pos = trajectory[-1, :3]
            delta = goal_pos - curr_pos_3d
            dist = np.linalg.norm(delta)

            # Only orient if we are far enough away (prevent jitter at target)
            if dist > 0.1:
                # Calculate required orientation (quaternion)
                # Align Body X (1,0,0) with Target Vector
                target_quat = self._get_alignment_quaternion(
                    curr_pos_3d, goal_pos, current_quat
                )

                # print(f"DEBUG: Guidance Override Active! Target Quat: {target_quat}, Dist: {dist:.2f}")

                # Override orientation in trajectory
                # We apply this to the whole horizon to encourage immediate rotation
                trajectory[:, 3:7] = target_quat

                # Zero out angular velocity targets to prevent spinning through target
                trajectory[:, 10:13] = 0.0

        return trajectory

    def _get_alignment_quaternion(
        self,
        ...
        """
        Calculate quaternion to align Body +X with vector to target.
        ...
        if norm < 1e-6:
            return np.array([1.0, 0.0, 0.0, 0.0])  # Identity (w, x, y, z)

        u = direction / norm

        def _align_to(vec: np.ndarray) -> np.ndarray:
            # Body X axis
            v_body = np.array([1.0, 0.0, 0.0])

            axis = np.cross(v_body, vec)
            axis_norm = np.linalg.norm(axis)
            dot = np.dot(v_body, vec)

            if axis_norm < 1e-6:
                if dot > 0:
                    return np.array([1.0, 0.0, 0.0, 0.0])
                # 180° rotation about Z (any orthogonal axis works)
                return np.array([0.0, 0.0, 0.0, 1.0])

            angle = np.arccos(np.clip(dot, -1.0, 1.0))
            rot_vec = (axis / axis_norm) * angle
        ...
        if self.use_symmetric_angle_error:
            return closest_symmetric_quat(q_pos, current_quat)

        q_neg = _align_to(-u)
        err_pos = quat_angle_error(q_pos, current_quat)
        err_neg = quat_angle_error(q_neg, current_quat)
        return q_pos if err_pos <= err_neg else q_neg
    ...
    def update_target_state(
        ...
        enable_waypoint = (
            self.mission_state.enable_waypoint_mode or self.mission_state.enable_multi_point_mode
        )

        if enable_waypoint:
            target_state = self._handle_multi_point_mode(
                current_position, current_quat, current_time
            )
            return self._apply_symmetry_to_target_state(target_state, current_quat)

        # DXF shape mode (V4.0.0: mission_state required)
        is_dxf = self.mission_state.dxf_shape_mode_active

        if is_dxf:
            target_state = self._handle_dxf_shape_mode(
                current_position, current_quat, current_time
            )
            return self._apply_symmetry_to_target_state(target_state, current_quat)
    ...
    def _handle_multi_point_mode(
        ...
        ang_error = self._quat_angle_error(target_state[3:7], current_quat)
    ...
    def _dxf_positioning_phase(
        ...
        if pos_error < self.position_tolerance and ang_error < self.angle_tolerance:
            if self.shape_stabilization_start_time is None:
                ...
                logger.info(f" Reached starting position, stabilizing for " f"{stab_time:.1f}s...")
            else:
    ...
    def _dxf_path_stabilization_phase(
        ...
        if is_end_stabilization:
            ...
            target_pos = getattr(self.mission_state, "dxf_current_target_position", None)
            path_s = 0.0
    ...
    def _dxf_returning_phase(
        ...
        return_pos = getattr(self.mission_state, "dxf_return_position", (0.0, 0.0))
        return_angle = getattr(self.mission_state, "dxf_return_angle", (0.0, 0.0, 0.0)) or (
            0.0,
            0.0,
            0.0,
        )
