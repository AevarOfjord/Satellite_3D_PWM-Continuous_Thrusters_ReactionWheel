...
from src.satellite_control.utils.navigation_utils import angle_difference, normalize_angle
...
class SimulationStateValidator:
    ...
    def __init__(
        ...
        self.use_symmetric_angle_error = False
        ...
        if app_config and app_config.actuator:
            if app_config.actuator.mode in [
                ActuatorMode.REACTION_WHEELS,
                ActuatorMode.ONE_THRUSTER_RW,
            ]:
                self.use_symmetric_angle_error = True
    ...
    def check_target_reached(self, current_state: np.ndarray, target_state: np.ndarray) -> bool:
        ...
        # Angle error (Quaternion)
        q1 = current_state[3:7]
        q2 = target_state[3:7]
        if self.use_symmetric_angle_error:
            ang_error = quat_angle_error_symmetric(q2, q1)
        else:
            ang_error = quat_angle_error(q2, q1)
