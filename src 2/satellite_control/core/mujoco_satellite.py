...
class MuJoCoSatelliteSimulator(SimulationBackend):
    ...
    def __init__(
        ...
        # V4.1.0: Support dynamic thruster configuration from ActuatorConfig
        if hasattr(self._app_config, "actuator") and self._app_config.actuator:
            # Use the new ActuatorConfig source of truth
            positions = self._app_config.actuator.thrusters.positions
            directions = self._app_config.actuator.thrusters.directions
            # Convert to dictionary with 1-based indexing for internal consistency
            self.thrusters = {i + 1: pos for i, pos in enumerate(positions.values())}
            self.thruster_directions = {i + 1: dir for i, dir in enumerate(directions.values())}
        ...
        self.thruster_forces = (
            self._physics_params.thruster_forces.copy()
        )  # This might need update if per-thruster force varies
    ...
    def set_actuator_ctrl(self, ctrl: np.ndarray):
        """
        Set actuator control inputs (data.ctrl).
        Used for Reaction Wheels and other direct actuators.

        Args:
            ctrl: Control vector matching model actuators
        """
        if len(ctrl) > self.model.nu:
            # Warn but truncate/clip?
            pass

        # Determine number of controls to set
        n_io = min(len(ctrl), self.model.nu)
        self.data.ctrl[:n_io] = ctrl[:n_io]
    ...
    def calculate_forces_and_torques(self):
        ...
        # Calculate net force and torque
        # V4.1.0: Use dynamic thruster count
