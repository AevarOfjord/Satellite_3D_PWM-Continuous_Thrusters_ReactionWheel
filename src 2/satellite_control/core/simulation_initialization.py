...
class SimulationInitializer:
    ...
    def _initialize_satellite_physics(
        ...
        if self.simulation_config is None:
            raise ValueError("simulation_config is required (V4.0.0: no SatelliteConfig fallback)")

        # V4.0.0: Pass app_config to MuJoCoSatelliteSimulator
        self.simulation.satellite = SatelliteThrusterTester(
            model_path=self.simulation_config.app_config.actuator.model_path,
    ...
    def _initialize_tracking_variables(self) -> None:
        ...
        control_dim = (
            self.simulation_config.app_config.actuator.control_dimension
            if self.simulation_config and hasattr(self.simulation_config, "app_config")
            else self.simulation.num_thrusters
        )
        self.simulation.previous_thrusters = np.zeros(control_dim, dtype=np.float64)
    ...
    def _initialize_mpc_controller(self, app_config: Any) -> None:
        """Initialize MPC controller."""
        logger.info(f"Initializing MPC Controller (Mode: {app_config.actuator.mode})...")
        ...
        if app_config.actuator.mode in [ActuatorMode.REACTION_WHEELS, ActuatorMode.ONE_THRUSTER_RW]:
            self.simulation.mpc_controller = ReactionWheelMPCController(
                actuator_config=app_config.actuator,
                prediction_horizon=app_config.mpc.prediction_horizon,
                dt=app_config.mpc.dt,
                satellite_mass=app_config.physics.total_mass,
                satellite_inertia=app_config.physics.moment_of_inertia,
                solver_time_limit=app_config.mpc.solver_time_limit,
                mpc_params=app_config.mpc,
            )
        else:
    ...
    def _log_initialization_summary(
        ...
        logger.info(
            f"INFO: Control update rate: " f"{1 / self.simulation.control_update_interval:.1f} Hz"
        )
        ...
        if app_config.physics.use_realistic_physics:
            ...
            logger.info(
                f"WARNING: - Ramp-up time: " f"{self.simulation.THRUST_RAMPUP_TIME * 1000:.0f} ms"
            )
            logger.info(
            ...
            logger.info(f"WARNING: - Position noise: " f"{position_noise_std * 1000:.2f} mm std")
        ...
        # V3.0.0: Obstacles removed from MissionState for now
        # mission_state = self.simulation_config.mission_state
        # if hasattr(mission_state, "obstacles_enabled") and mission_state.obstacles_enabled:
