...
class SimulationLogger:
    ...
    def log_step(
        ...
        # Thruster-only view for mixed RW + thruster control vectors
        thruster_vector = thruster_action
        if len(thruster_action) in [4, 5, 9]:
            thruster_vector = thruster_action[3:]

        command_vector_binary = (thruster_vector > 0.5).astype(int)
        ...
        # Active Thrusters
        total_active_thrusters = int(np.sum(thruster_vector > 0.01))

        # Switches
        thruster_switches = 0
        if previous_thruster_action is not None:
            prev_thruster_vector = previous_thruster_action
            if len(previous_thruster_action) in [4, 5, 9]:
                prev_thruster_vector = previous_thruster_action[3:]
            if len(prev_thruster_vector) == len(thruster_vector):
                thruster_switches = int(
                    np.sum(np.abs(thruster_vector - prev_thruster_vector) > 0.01)
                )
