...
class SatelliteMPCLinearizedSimulation:
    ...
    def log_simulation_step(
        ...
        # Handle mixed Reaction Wheel + Thruster modes
        if len(thruster_action) == 4:
            # ONE_THRUSTER_RW (legacy): [RW1, RW2, RW3, THR_MAIN]
            # Thruster is at index 3 (ID 1)
            active_thrusters = [1] if thruster_action[3] > 0.01 else []
        elif len(thruster_action) == 5:
            # ONE_THRUSTER_RW: [RW1, RW2, RW3, THR_MAIN, THR_BRAKE]
            active_thrusters = []
            if thruster_action[3] > 0.01:
                active_thrusters.append(1)
            if thruster_action[4] > 0.01:
                active_thrusters.append(2)
        elif len(thruster_action) == 9:
            # REACTION_WHEELS: [RW1, RW2, RW3, THR1..6]
            # Thrusters at indices 3..8 match IDs 1..6
            active_thrusters = [
                i - 2 for i, val in enumerate(thruster_action) if i >= 3 and val > 0.01
            ]
        else:
            # Standard 12 thrusters
    ...
    def update_mpc_control(self) -> None:
        ...
        if self.simulation_time >= self.next_control_simulation_time:
            ...
            # Dispatch control to actuators
            # Handle mixed Reaction Wheel + Thruster modes
            if len(thruster_action) in [4, 5] and hasattr(self.satellite, "set_actuator_ctrl"):
                # RW + thruster(s) mode
                # Model actuators order: [rw_x, rw_y, rw_z, thruster_main_force, thruster_brake_force]
                # Control vector order: [0-2] = RW Torque, [3:] = Thruster Forces
                self.satellite.set_actuator_ctrl(thruster_action)

            elif len(thruster_action) == 9 and hasattr(self.satellite, "set_actuator_ctrl"):
                # REACTION_WHEELS mode (6 Thrusters + 3 RW)
                # [0-2] = RW Torque, [3-8] = Thruster Force
                rw_cmd = thruster_action[:3]
                thrust_cmd = thruster_action[3:]
                self.satellite.set_actuator_ctrl(rw_cmd)
                self.set_thruster_pattern(thrust_cmd)

            else:
                # Legacy / Standard Thruster-only mode
            ...
            if (
                ...
                and self.simulation_config.app_config.actuator.mode
                in [ActuatorMode.REACTION_WHEELS, ActuatorMode.ONE_THRUSTER_RW]
            ):
                ang_error = quat_angle_error_symmetric(
            ...
            # Determine active thrusters for display
            if len(display_thrusters) == 4:
                active_thruster_ids = [1] if display_thrusters[3] > 0.01 else []
            elif len(display_thrusters) == 5:
                active_thruster_ids = []
                if display_thrusters[3] > 0.01:
                    active_thruster_ids.append(1)
                if display_thrusters[4] > 0.01:
                    active_thruster_ids.append(2)
            elif len(display_thrusters) == 9:
                active_thruster_ids = [
                    i - 2 for i, val in enumerate(display_thrusters) if i >= 3 and val > 0.01
                ]
            else:
                active_thruster_ids = [int(x) for x in np.where(display_thrusters > 0.01)[0] + 1]

            # Note: active_thruster_ids is already appended to command_history in log_simulation_step
            # self.command_history.append(active_thruster_ids)

            # Helper for clean state formatting - returns (position_str, angle_str) tuple
            def fmt_state(s):
                x_mm = s[0] * 1000
                y_mm = s[1] * 1000
                z_mm = s[2] * 1000
                ...
                pos_str = f"[x:{x_mm:.0f}, y:{y_mm:.0f}, z:{z_mm:.0f}]mm"
                ang_str = f"[Roll:{roll_deg:.1f}, Pitch:{pitch_deg:.1f}, Yaw:{yaw_deg:.1f}]°"
            ...
            if len(display_thrusters) == 4:
                # Map ID 1 to Index 3
                thr_out = [round(float(display_thrusters[3]), 2) for _ in active_thruster_ids]
            elif len(display_thrusters) == 5:
                # Map IDs 1-2 to Indices 3-4
                thr_out = []
                for thr_id in active_thruster_ids:
                    idx = 3 if thr_id == 1 else 4
                    thr_out.append(round(float(display_thrusters[idx]), 2))
            elif len(display_thrusters) == 9:
                # Map ID i to Index i+2
                thr_out = [round(float(display_thrusters[i + 2]), 2) for i in active_thruster_ids]
            else:
                thr_out = [round(float(display_thrusters[i - 1]), 2) for i in active_thruster_ids]

            # Reaction Wheel Status
            rw_status = ""
            if len(display_thrusters) in [4, 5, 9]:
                # Format: [Tx, Ty, Tz]
                rw_torques = [round(float(val), 3) for val in display_thrusters[:3]]
                rw_status = f"     RW Torques = {rw_torques}\n"

            curr_pos, curr_ang = fmt_state(current_state)
            targ_pos, targ_ang = fmt_state(safe_target)
