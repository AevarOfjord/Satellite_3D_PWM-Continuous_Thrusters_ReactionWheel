...
### What Changed
The custom C++ physics engine replaces the legacy Python simulator as the default for simulation. Visualization/replay is handled outside the physics backend.
...
- **Dependencies:** Removes the legacy Python simulator as a runtime dependency
...
| Metric | V1.x | V2.2.0 |
|--------|------|--------|
| Physics Engine | Legacy Python engine | C++ (custom) |
